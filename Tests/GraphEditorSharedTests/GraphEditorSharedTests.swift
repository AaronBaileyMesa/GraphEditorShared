...
struct GraphEditorSharedTests {
    ...
    @Test func testNodeContentDisplayText() {
        #expect(NodeContent.string("LongStringHere").displayText == "LongString…", "Truncates long string")  // Proper ellipsis
        #expect(NodeContent.number(3.14159).displayText == "3.1", "Formats number")
        let date = Date(timeIntervalSince1970: 0)
        #expect(NodeContent.date(date).displayText == "1/1/70", "Formats date (locale-dependent; adjust expectation if needed)")
    }
